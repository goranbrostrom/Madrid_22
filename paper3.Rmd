...
## New:
infdat$summer <- infdat[infdat$week %in% 14:39, ]
...
fit.cr <- coxreg(Surv(enter, exit, event) ~  highTemp + aver + emeantemp + 
                     sex + subreg + illeg + parity + socBranch,
                 data = winter)
dr <- drop1(fit.cr, test = "Chisq")
ltx2(fit.cr, dr = dr, caption = "Neonatal mortality, October to March. Adjusted for social branch, sex, illegitimacy, parity, time period, and subregion.", 
...
fit.cr <- coxreg(Surv(enter, exit, event) ~  aver + emeantemp + 
                 ...
                 data = summer)
dr <- drop1(fit.cr, test = "Chisq")
ltx2(fit.cr, dr = dr, caption = "Neonatal mortality, April to September. Adjusted for social branch, sex, illegitimacy, parity, time period, and subregion.", 
...
dr <- drop1(fit.tpch, test = "Chisq")
ltx2(fit.tpch, dr = dr, caption = "Postneonatal mortality, April to September. Adjusted for social branch, sex, illegitimacy, parity, time period, and subregion.", label = "tab:postanals", keep = NULL)
```
...
*without* the "log-cube transform". A first try is fit a *Weibull* distribution to the time
to death for an infant alive at age 28 /365 years (28 days), that is, time is measured from
age 28 days until death or time reaches 365 - 28 days, when follow-up is censored.
In **R** language, with the package *eha* [@eha], we try

```
births <- readRDS("data/births.rds")
pbirths <- age.window(births, c(28 / 365, 1)
fit.w <- phreg(Surv(enter - 28/365, exit - 28/365, event) ~ 1, data = pbirths)
fit <- coxreg(Surv(enter - 28/365, exit - 28/365, event) ~ 1, data = pbirths)
```

where `phreg` performs the Weibull regression (no covariates) and `coxreg` the Cox regression. In 
Figure \@ref(fig:bpprim) the result is shown in the form of the two cumulative hazards estimates.

```{r bpprim, fig.cap = "Postneonatal survival, Weibull vs. Nelson-Aalen."}
births <- readRDS("data/births.rds")
pbirths <- age.window(births, c(28 / 365, 1))
fit.w <- phreg(Surv(enter - 28 / 365, exit - 28 / 365, event) ~ 1, data = pbirths)
fit <- coxreg(Surv(enter - 28 / 365, exit - 28 / 365, event) ~ 1, data = pbirths)
...
Not a very good fit. However, we need a *left-truncated Weibull distribution*, that is, assuming that
*exogeneous infant mortality* is Weibull distributed.

```
fit.w <- phreg(Surv(enter, exit, event) ~ 1, data = pbirths)
fit <- coxreg(Surv(enter, exit, event) ~ 1, data = pbirths)
```
Note the difference: No subtraction of the 28 days from the time variables, which implies that the time origin
is *birth* and *not* the 28th day of life. Thus our data are *left truncated* at 28 days, and the 
Weibull attempt estimates neonatal exogeneous mortality through *extrapolation*. The Cox regression approach
is a non-parametric procedure, and thus not capable of any extrapolated estimation, so neonatal
mortality will simply be taken as zero.


```{r ltruw, fig.cap = "Postneonatal survival, truncated Weibull vs. Nelson-Aalen."}
fit.w <- phreg(Surv(enter, exit, event) ~ 1, data = pbirths)
fit <- coxreg(Surv(enter, exit, event) ~ 1, data = pbirths)
oldpar <- par(mfrow = c(1, 2))
##check.dist(fit, fit.w)
cc <- hazards(fit)
plot(cc[[1]][, 1], cumsum(cc[[1]][, 2]), type = "l", col = "blue", ylim = c(0, 0.07), axes = FALSE,
     xlab = "Age", ylab = "Cumulative hazards")
axis(1, at = c(28 / 365, 1), labels = c("28/365", "1"))
axis(2, las = 1)
box()
xx <- hazards(fit.w)
lines(xx$x, xx$y, col = "red", lty = 2)
plot(cc[[1]][, 1], cumsum(cc[[1]][, 2]), type = "l", col = "blue", ylim = c(0, 0.07), axes = FALSE,
     xlab = "Age", ylab = "Cumulative hazards")
##plot(fit, fn = "cum", col = "blue", ylim = c(0, 0.07), axes = FALSE)
axis(1, at = c(28 / 365, 1), labels = c("28/365", "1"))
axis(2, las = 1)
box()
...
```
 ...
 Look here (Figure \@ref(fig:ltruw))! A perfect fit, 
just like the one under the $g$- transform. But the Weibull curve
does not start at zero (left hand panel), however it is parallell 
to the Nelson-Aalen estimator, and the right hand panel shows what happens if we subtract 
the start value from the Weibull curve and redraw. 

## The biometric analysis of  exogeneous and endogeneous infant mortality

So, finally, we show the full procedure how to perform the "biometric analysis" *without*
the log-cube transform. It simply consists of two steps: 

1.    Run a Cox regression on the full data set.
2.    Run a *left truncated* (at 28 days) Weibull regression.
3.    Plot the two cumulative hazards functions in the same graph and read
      their crossings of the vertical line at age = 1 year (365 days). Figure \@ref(fig:crossings).
     
```{r crossings, fig.cap = "Biometric analysis.", fig.height = 4}
fit.c <- coxreg(Surv(enter, exit, event) ~ 1, data = births)
fit.w <- phreg(Surv(enter, exit, event) ~ 1, data = pbirths)
cc <- hazards(fit.c)
plot(c(0, cc[[1]][, 1]), c(0, cumsum(cc[[1]][, 2])), type = "l", col = "blue", axes = FALSE,
     xlab = "Age (days)", ylab = "Cumulative hazards", ylim = c(0, 0.08))
axis(1, at = c(0, 28 / 365, 1), labels = c(0, 28, 365))
axis(2, las = 1, at = round(c(0, max(cumsum(cc[[1]][, 2])), max(xx$y)), 3))
##abline(v = 1)
box()
xx <- hazards(fit.w)
lines(xx$x, xx$y, col = "red", lty = 2)
...
abline(h = c(max(cumsum(cc[[1]][, 2])), max(xx$y)), lty = 3)
```

So, total mortality is 0.076, exogeneous mortality is 0.063, and endogeneous mortality
is the difference, 0.013, or 17 per cent of the total mortality.

Now, try the original "biometric" approach and compare *(Exercise)*.
