...
abstract: "The effect of extreme temperatures on infant mortality in the Umeå and Skellefteå regions 1895-1950 is studied in a biometric analysis setting. More precisely, the effect of climate and weather, measured by temperature, average and extremes, on infant mortality is investigated. It turns out that climate (average) is more important than weather (extremes), low average temperatures are more important than temporary dips in temperature."
---
...
##fit.all <- coxph(Surv(enter, exit, event) ~ strata(ageIvl) * (aver + emeantemp + lowTemp + highTemp + socBranch + ##socStatus + sex + period),
  ##               data = all)
##dr.all <- drop1(fit.all, test = "Chisq")

inftab <- toTpch(Surv(enter, exit, event) ~ subreg + sex + period + socBranch + socStatus + parity + 
                     aver + emeantemp + lowTemp + highTemp + summer + ageIvl, data = all,
...
fit.all <- tpchreg(oe(event, exposure) ~  strata(ageIvl) * (aver + emeantemp + lowTemp + highTemp + socBranch + socStatus + sex + period) + emeantemp, data = inftab, time = age)
dr.all <- drop1(fit.all, test = "Chisq")
##system.time(tabneo.fit <- tpchreg(oe(event, exposure) ~ aver + emeantemp + highTemp + lowTemp + socBranch + socStatus, 
  ##               data = tabneo, time = age))
##system.time(tabpost.fit <- tpchreg(oe(event, exposure) ~ aver + emeantemp + highTemp + lowTemp + socBranch + socStatus, 
...
```{r formneo}
formneo <- as.formula("oe(event, exposure) ~  lowTemp + highTemp + aver + emeantemp + 
                     sex + subreg + parity + socStatus + period")
```
...
fit.cr <- tpchreg(formneo, data = winter, time = age)
...
##neo$exit <- g(neo$exit * 365) # Skip the log-cube transformation
##neo$enter <- g(neo$enter * 365)
fit.cr <- tpchreg(formneo, data = tabneo[tabneo$summer, ], time = age)
dr <- drop1(fit.cr, test = "Chisq")
...
##tpneo <- toTpch(Surv(enter, exit, event) ~ lowTemp + highTemp + aver + emeantemp + sex + socBranch + illeg + parity
  ##              + subreg + period, data = infdat, cuts = c(0, 1))
##winter <- gtpneo[gtpneo$summer == 0, ]
##winter$season <- factor(winter$season)

fit.tpch <- tpchreg(oe(event, exposure) ~ period * (lowTemp + highTemp + aver + emeantemp) + sex + socBranch  + parity + subreg + period, data = tabpost[!tabpost$summer, ], time = age)
dr <- drop1(fit.tpch, test = "Chisq")
tbl(round(dr, 4), caption = "Interaction tests with period, postneonatal mortality.", rownames = TRUE)
...
```{r setformw}
form <- as.formula("oe(event, exposure) ~ lowTemp + highTemp + aver + emeantemp + sex + socStatus + parity + subreg")
```


#### The time period 1895--1913

```{r postw1, results = "asis"}
fit.pw1 <- tpchreg(form, data = tabpost[(!tabpost$summer) & tabpost$period == "(1895,1914]", ], time = age)
dr <- drop1(fit.pw1, test = "Chisq")
ltx2(fit.pw1, dr = dr, caption = "Postneonatal mortality, October to March 1895-1914. Adjusted for social branch, sex, illegitimacy, parity, and subregion.", label = "tab:postw1", keep = NULL)
```
...
```{r postw2, results = "asis"}
fit.pw2 <- tpchreg(form, data = tabpost[(!tabpost$summer) & tabpost$period == "(1914,1935]", ], time = age)
dr2 <- drop1(fit.pw2, test = "Chisq")
ltx2(fit.pw2, dr = dr2, caption = "Postneonatal mortality, October to March 1914-1934. Adjusted for social branch, sex, illegitimacy, parity, and subregion.", label = "tab:postw2", keep = NULL)
```
...
```{r postw3, results = "asis"}
fit.pw3 <- tpchreg(form, data = tabpost[(!tabpost$summer) & tabpost$period == "(1935,1951]", ], time = age)
dr3 <- drop1(fit.pw3, test = "Chisq")
ltx2(fit.pw3, dr = dr3, caption = "Postneonatal mortality, October to March 1935-1950. Adjusted for social branch, sex, illegitimacy, parity, and subregion.", label = "tab:postw3", keep = NULL)
```
