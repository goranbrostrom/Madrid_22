...
We can see that the variables *emeantemp*, *lowTemp*, *socStatus* and *period* all have different effects on mortality in the neonatal case compared to the postneonatal case. In terms of 
...
This shows that the temperature effects do not vary with time period,
...
```{r setformw}
form <- as.formula("oe(event, exposure) ~ lowTemp + highTemp + extemp + emeantemp + socBranch + subreg + sex + parity")
```
...
ltx2(fit.postw1, dr = dr.postw1, caption = "Postneonatal mortality, October to March 1895-1914. Adjusted for social branch, sex, parity, and subregion.", label = "tab:postw1", keep = 5)
```
