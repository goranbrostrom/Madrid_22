...
inftab <- toTpch(Surv(enter, exit, event) ~ subreg + sex + birthmonth + period + socBranch + socStatus + parity + 
...
```{r shownonprop}
tbl(dr.all, caption = "Proportionality tests, neonatal vs. postneonatal.", rownames = TRUE)
```
...
fit.cr <- coxreg(Surv(enter, exit, event) ~  highTemp + aver + emeantemp + 
                     sex + subreg + illeg + parity + socBranch + period,
                 data = winter)
...
##neo$exit <- g(neo$exit * 365) # Skip the log-cube transformation
##neo$enter <- g(neo$enter * 365)
fit.cr <- tpchreg(oe(event, exposure) ~  aver + emeantemp + 
                     sex + subreg  + socBranch + period,
                 data = inftab[inftab$summer, ], time = age)
dr <- drop1(fit.cr, test = "Chisq")
...
fit.tpch <- tpchreg(oe(event, exposure) ~ lowTemp + highTemp + aver + emeantemp + sex + socBranch  + 
                        parity + subreg + period + summer, data = tabpost[!tabpost$summer, ], time = age)
