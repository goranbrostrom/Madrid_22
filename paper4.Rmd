...
if (!(file.exists("data/inftab.rds") && file.exists("data/tabneo.rds") && file.exists("data/tabpost.rds"))){
    ...
    inftab <- toTpch(Surv(enter, exit, event) ~ subreg + sex + period + socBranch + socStatus + urban + parity + 
...
fit.all <- tpchreg(oe(event, exposure) ~  strata(ageIvl) * 
                       (extemp + extemp.1 + emeantemp + socBranch + socStatus + period + urban), 
...
```{r formneo}
formneo <- as.formula("oe(event, exposure) ~  extemp + extemp.1 + emeantemp + socBranch + socStatus + period + urban +
                     subreg + sex + parity")
...
```{r setformw}
form <- as.formula("oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + socStatus + urban + subreg + sex + parity")
form.sb <-  as.formula("oe(event, exposure) ~ (extemp + extemp.1 + emeantemp) * socBranch + socStatus + urban + subreg + sex + parity")
form.sr <- as.formula("oe(event, exposure) ~ (extemp + extemp.1 + emeantemp) * subreg + socBranch + socStatus + urban + sex + parity") 
...
##tpneo <- toTpch(Surv(enter, exit, event) ~ lowTemp + highTemp + extemp + emeantemp + sex + socBranch + socStatus + illeg + parity
...
fit.postw1sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + socStatus + urban + sex + parity, 
...
fit.postw1srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + socStatus + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1895,1914]" & tabpost$subreg != "ume", ], time = age)
...
fit.postw2sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + socStatus + urban + sex + parity, 
...
fit.postw2srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + socStatus + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1914,1935]" & tabpost$subreg != "ume", ], time = age)
...
fit.postw3 <- tpchreg(oe(event, exposure) ~ extemp.1 + emeantemp + socBranch + socStatus + urban + subreg + sex + parity, 
...
fit.postw3sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + socStatus + urban + sex + parity, 
...
fit.postw3srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + socStatus + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1935,1951]" & tabpost$subreg != "ume", ], time = age)
...
fit.posts <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + socStatus + urban + period + 
                        subreg + parity + sex, 
                    data = tabpost[tabpost$summer, ], time = age)
