...
generated great interest over the last years, see @tbgb10 for an example of our own efforts.

The effect of seasonal variation and the occurrence of extreme monthly temperatures
is studied and interacted with social class and time period. Studies are performed 
...
```{r pneodata, fig.cap = "Crude postneonatal mortality by week of year, Umeå/Skellefteå 1895--1950.", fig.height = 3, cache = TRUE}
...
```{r ntrend, fig.cap = "Crude NMR by year, Umeå-Skellefteå 1895--1950.", fig.height = 4, cache = TRUE}
...
```{r pntrend, fig.cap = "Crude postneonatal mortality by year, Umeå-Skellefteå 1895--1950.", fig.height = 4, cache = TRUE}
...
tbl(head(infdat[, c("enter", "exit", "event", "extemp", "extemp.1", "emeantemp", "week", "year")]),
...
if (!(file.exists("data/inftab.rds") && file.exists("data/tabneo.rds") && file.exists("data/tabpost.rds"))){
    ...
    inftab <- toTpch(Surv(enter, exit, event) ~ subreg + sex + period + socst + urban + parity + 
                         extemp + extemp.1 + emeantemp + summer + ageIvl, data = all,
...
fit.all <- tpchreg(oe(event, exposure) ~  strata(ageIvl) * 
                       (extemp + extemp.1 + emeantemp + socst + period + urban), 
...
We can see that the variables *emeantemp*, *socStatus* and *period* all have different effects on mortality in the neonatal case compared to the postneonatal case. In terms of 
*infant mortality*, we can say that these caovariates have non-proportional effects.
...
```{r formneo}
formneo <- as.formula("oe(event, exposure) ~  extemp + extemp.1 + emeantemp + socst + period + urban +
                     subreg + sex + parity")
tabneo <- readRDS("data/tabneo.rds")
```
...
```{r neoanalwp, results = 'asis', cache = TRUE}
...
ltx2(fit.neow, dr = dr, caption = "Neonatal mortality, October to March.", 
     label = "tab:neoanalw", keep = NULL)
```

#### Temperature and sex

We check the interaction between temperature and sex and period.

```{r neosexper}
fit <- tpchreg(oe(event, exposure) ~ sex * (extemp + extemp.1 + emeantemp), 
               time = age, data = winter)
dr <- drop1(fit, test = "Chisq")
dr
```

Obviously no temperature and sex interactions
...
ltx2(fit.neos, dr = dr.neos, caption = "Neonatal mortality, April to September.", 
...
```{r setformw}
form <- as.formula("oe(event, exposure) ~ socst + extemp + extemp.1 + emeantemp + urban + subreg + sex + parity")
formex <- as.formula("oe(event, exposure) ~ socst + socst:extemp + extemp.1 + emeantemp + urban + subreg + sex + parity")
formex1 <- as.formula("oe(event, exposure) ~ socst + socst:extemp.1 + extemp + emeantemp + urban + subreg + sex + parity")
formem <- as.formula("oe(event, exposure) ~ socst + socst:I(emeantemp + extemp) + extemp.1 + urban + subreg + sex + parity")
form.sb <-  as.formula("oe(event, exposure) ~ (extemp + extemp.1 + emeantemp) * socst + urban + subreg + sex + parity")
form.sr <- as.formula("oe(event, exposure) ~ (extemp + extemp.1 + emeantemp) * subreg + socst + urban + sex + parity") 
tabpost <- readRDS("data/tabpost.rds")
```

#### By birth month

See Figure \@ref(fig:bybmonth).

```{r bybmonth, fig.cap = "Crude postneonatal relative (to January = 1) mortality by birth month.", cache = TRUE}
pn <- age.window(infdat, c(28/365, 1))
fit <- coxreg(Surv(enter, exit, event) ~ as.factor(birthmonth), data = pn)
plot(exp(c(0, coef(fit))), type = "b", xlab = "Birth month", ylab = "Hazard ratios",
     col = "blue")
abline(h = 1)
```
...
```{r postanalwp, results='asis', cache = TRUE}
##tpneo <- toTpch(Surv(enter, exit, event) ~ lowTemp + highTemp + extemp + emeantemp + sex + socst + illeg + parity
  ##              + subreg + period, data = infdat, cuts = c(0, 1))
##winter <- gtpneo[gtpneo$summer == 0, ]
##winter$season <- factor(winter$season)

fit.postw <- tpchreg(oe(event, exposure) ~ period * (extemp + extemp.1 + emeantemp), 
                     data = tabpost[!tabpost$summer, ], time = age)
dr.postw <- drop1(fit.postw, test = "Chisq")
```

Interaction tests with temperature and period, postneonatal mortality:

```{r postanalw, cache = FALSE}
round(dr.postw, 4)
#tbl(round(dr.postw, 4), caption = "Interaction tests with temperature and period, postneonatal ##mortality.", rownames = TRUE)
```

This shows that the temperature effects do not vary much with time period,
but we continue with separate analyses for the time periods `r levels(tabpost$period)`. Just
to make sure, will be joined when appropriate.

#### Sex, urban,  and temperature


```{r postanalwpsex, results='asis', cache = TRUE}
##tpneo <- toTpch(Surv(enter, exit, event) ~ lowTemp + highTemp + extemp + emeantemp + sex + socst + illeg + parity
  ##              + subreg + period, data = infdat, cuts = c(0, 1))
##winter <- gtpneo[gtpneo$summer == 0, ]
##winter$season <- factor(winter$season)

fit.postwsex <- tpchreg(oe(event, exposure) ~ (sex + urban) * (extemp + extemp.1 + emeantemp), 
                     data = tabpost[!tabpost$summer, ], time = age)
dr.postwsex <- drop1(fit.postwsex, test = "Chisq")
```

Interaction tests with temperature and sex, postneonatal mortality:

```{r postanalwsex, cache = FALSE}
round(dr.postwsex, 4)
#tbl(round(dr.postw, 4), caption = "Interaction tests with temperature and period, urban, postneonatal ##mortality.", rownames = TRUE)
```

Obviously no interactions between temperature and sex, and urban.
...
ltx2(fit.postw1, dr = dr.postw1, caption = "Postneonatal mortality, October to March 1895-1914.", 
     label = "tab:postw1", keep = NULL)
```

Table \@ref(tab:postw1) shows that the effects of  extreme temperatures have the expected directions, extra high temperatures is *positive* 
(remember that this concerns winter conditions), but not statistically significant. *Climate* (emeantemp), on the other hand shows to be very
important, where increasing temperature by one degree lower mortality by 0.7 per cent. The normal short term fluctuations (extemp) are not so important, 
but in the expected direction. 

The importance of social class (socBranch) in explaining the effects of climate and weather is negligible, but there is variation between
the regions regarding *climate*: 

```{r postw1srpu, cache = TRUE}
fit.postw1sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1895,1914]" & tabpost$subreg == "ume", ], time = age)
dr.postw1sru <- drop1(fit.postw1sru, test = "Chisq")
```

```{r postw1sru, results = "asis"}
ltx2(fit.postw1sru, dr = dr.postw1sru, caption = "Postneonatal mortality, Umeå, October to March 1895-1913.",
     label = "tab:postw1sru", keep = NULL)
```

```{r postw1srps, cache = TRUE}
fit.postw1srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1895,1914]" & tabpost$subreg != "ume", ], time = age)
dr.postw1srs <- drop1(fit.postw1srs, test = "Chisq")
```

```{r postw1srs, results = "asis"}
ltx2(fit.postw1srs, dr = dr.postw1srs, caption = "Postneonatal mortality, Skellefteå with inland, October to March 1895-1913.", 
     label = "tab:postw1srs", keep = NULL)
```

We can conclude from Tables \@ref(tab:postw1sru) and \@ref(tab:postw1srs) that the main difference between Umeå and Skellefteå is that
*climate* has more severe effect in Skellefteå (with inland), while *weather* is less important.

The effects of temperature in different social classes are best shown graphically, see 
Figure \@ref(fig:soc2).

```{r soc1, fig.cap = "Interactions between social status and temperature, winter 1895--1913."}
source("R/socfig.R")
oldpar <- par(mfrow = c(1,3))
tempdat <- tabpost[(!tabpost$summer) & tabpost$period == "(1895,1914]", ]
fit <- tpchreg(formem, data = tempdat, time = age)
fitex <- tpchreg(formex, data = tempdat, time = age)
fitex1 <- tpchreg(formex1, data = tempdat, time = age)
socfig(coef(fit), main = "Weekly mean temp")
socfig(coef(fitex), main = "Excess temp", ran = c(-10, 10))
socfig(coef(fitex1), main = "Lagged excess temp", ran = c(-10, 10))
par(oldpar)
```
...
ltx2(fit.postw2, dr = dr.postw2, caption = "Postneonatal mortality, October to March 1914-1934. Adjusted for sex, parity, and subregion.", label = "tab:postw2", keep = NULL)
```

Table \@ref(tab:postw2) shows that the weather effect is lagged one week (*extemp.1*), and that climate continues to be very important.
Let's look at regional differences.

```{r postw2srpu, cache = TRUE}
fit.postw2sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1914,1935]" & tabpost$subreg == "ume", ], time = age)
dr.postw2sru <- drop1(fit.postw2sru, test = "Chisq")
```

```{r postw2sru, results = "asis"}
ltx2(fit.postw2sru, dr = dr.postw2sru, caption = "Postneonatal mortality, Umeå, October to March 1914-1934.",
     label = "tab:postw2sru", keep = NULL)
```

```{r postw2srps, cache = TRUE}
fit.postw2srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1914,1935]" & tabpost$subreg != "ume", ], time = age)
dr.postw2srs <- drop1(fit.postw2srs, test = "Chisq")
```

```{r postw2srs, results = "asis"}
ltx2(fit.postw2srs, dr = dr.postw2srs, caption = "Postneonatal mortality, Skellefteå with inland, October to March 1914-1934.", 
     label = "tab:postw2srs", keep = NULL)
```

The effects of temperature in different social classes are best shown graphically, see 
Figure \@ref(fig:soc2).

```{r soc2, fig.cap = "Interactions between social status and temperature, winter 1914--1934."}
source("R/socfig.R")
oldpar <- par(mfrow = c(1,3))
tempdat <- tabpost[(!tabpost$summer) & tabpost$period == "(1914,1935]", ]
fit <- tpchreg(formem, data = tempdat, time = age)
fitex <- tpchreg(formex, data = tempdat, time = age)
fitex1 <- tpchreg(formex1, data = tempdat, time = age)
socfig(coef(fit), main = "Weekly mean temp")
...
fit.postw3 <- tpchreg(oe(event, exposure) ~ extemp.1 + emeantemp + socst + urban + subreg + sex + parity, 
                      data = tabpost[(!tabpost$summer) & tabpost$period == "(1935,1951]", ], time = age)
...
ltx2(fit.postw3, dr = dr.postw3, caption = "Postneonatal mortality, October to March 1935-1950. Adjusted for sex, parity, and subregion.", label = "tab:postw3", keep = NULL)
```

Table \@ref(tab:postw3) shows that

```{r postw3srpu, cache = TRUE}
fit.postw3sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1935,1951]" & tabpost$subreg == "ume", ], time = age)
dr.postw3sru <- drop1(fit.postw3sru, test = "Chisq")
```

```{r postw3sru, results = "asis"}
ltx2(fit.postw3sru, dr = dr.postw3sru, caption = "Postneonatal mortality, Umeå, October to March 1935-1950.",
     label = "tab:postw3sru", keep = NULL)
```

```{r postw3srps, cache = TRUE}
fit.postw3srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1935,1951]" & tabpost$subreg != "ume", ], time = age)
dr.postw3srs <- drop1(fit.postw3srs, test = "Chisq")
```

```{r postw3srs, results = "asis"}
ltx2(fit.postw3srs, dr = dr.postw3srs, caption = "Postneonatal mortality, Skellefteå with inland, October to March 1895-1913.", 
     label = "tab:postw3srs", keep = NULL)
```

The effects of temperature in different social classes are best shown graphically, see 
Figure \@ref(fig:soc2).

```{r soc3, fig.cap = "Interactions between social status and temperature, winter 1935--1950."}
source("R/socfig.R")
oldpar <- par(mfrow = c(1,3))
tempdat <- tabpost[(!tabpost$summer) & tabpost$period == "(1935,1951]", ]
fit <- tpchreg(formem, data = tempdat, time = age)
fitex <- tpchreg(formex, data = tempdat, time = age)
fitex1 <- tpchreg(formex1, data = tempdat, time = age)
socfig(coef(fit), main = "Weekly mean temp")
...
```{r postanalsp, results='asis', cache = TRUE}
##summer <- gtpneo[gtpneo$summer == 1, ]
##winter$season <- factor(winter$season)

fit.posts <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + period + 
                        subreg + parity + sex, 
                    data = tabpost[tabpost$summer, ], time = age)
...
ltx2(fit.posts, dr = dr, caption = "Postneonatal mortality, April to September. Adjusted for sex, parity, time period, and subregion.", label = "tab:postanals", keep = NULL)
```

The result in Table \@ref(tab:postanals) shows that *climate* (`emeantemp`) is more important than 
*weather* (`exTemp`). Moreover, no signs of interaction between weather or climate and the rest of 
