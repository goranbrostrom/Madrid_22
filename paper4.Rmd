...
fit.all <- tpchreg(oe(event, exposure) ~  strata(ageIvl) * 
                       (extemp + extemp.1 + emeantemp + socBranch + period), 
...
```{r formneo}
formneo <- as.formula("oe(event, exposure) ~  extemp + extemp.1 + emeantemp + socBranch + period +
                     subreg + sex + parity")
...
```{r setformw}
form <- as.formula("oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + subreg + sex + parity")
form.sb <-  as.formula("oe(event, exposure) ~ (extemp + extemp.1 + emeantemp) * socBranch + subreg + sex + parity")
form.sr <- as.formula("oe(event, exposure) ~ (extemp + extemp.1 + emeantemp) * subreg + socBranch + sex + parity") 
...
fit.postwsex <- tpchreg(oe(event, exposure) ~ sex * (extemp + extemp.1 + emeantemp), 
                     data = tabpost[!tabpost$summer, ], time = age)
...
Obviously no interactions between temperature and sex.
...
fit.postw1sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + sex + parity, 
...
ltx2(fit.postw1sru, dr = dr.postw1sru, caption = "Postneonatal mortality, Umeå, October to March 1895-1913.",
     label = "tab:postw1sru", keep = 3)
```

```{r postw1srps, cache = TRUE}
fit.postw1srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1895,1914]" & tabpost$subreg != "ume", ], time = age)
...
ltx2(fit.postw2, dr = dr.postw2, caption = "Postneonatal mortality, October to March 1914-1934. Adjusted for sex, parity, and subregion.", label = "tab:postw2", keep = 3)
```
...
fit.postw2sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + sex + parity, 
...
ltx2(fit.postw2sru, dr = dr.postw2sru, caption = "Postneonatal mortality, Umeå, October to March 1914-1934.",
     label = "tab:postw2sru", keep = 3)
```

```{r postw2srps, cache = TRUE}
fit.postw2srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1914,1935]" & tabpost$subreg != "ume", ], time = age)
...
fit.postw3 <- tpchreg(oe(event, exposure) ~ extemp.1 + emeantemp + socBranch + subreg + sex + parity, 
...
ltx2(fit.postw3, dr = dr.postw3, caption = "Postneonatal mortality, October to March 1935-1950. Adjusted for sex, parity, and subregion.", label = "tab:postw3", keep = 3)
```

Table \@ref(tab:postw3) shows that

```{r postw3srpu, cache = TRUE}
fit.postw3sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + sex + parity, 
...
ltx2(fit.postw3sru, dr = dr.postw3sru, caption = "Postneonatal mortality, Umeå, October to March 1935-1950.",
     label = "tab:postw3sru", keep = 3)
```

```{r postw3srps, cache = TRUE}
fit.postw3srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$period == "(1935,1951]" & tabpost$subreg != "ume", ], time = age)
...
fit.posts <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socBranch + period + 
                        subreg + parity + sex, 
                    data = tabpost[tabpost$summer, ], time = age)
...
ltx2(fit.posts, dr = dr, caption = "Postneonatal mortality, April to September. Adjusted for sex, parity, time period, and subregion.", label = "tab:postanals", keep = 3)
```
