...
g <- function(x) (log(1 + x))^3 # x is time in days! #### Note ###
```

# Introduction

The only (so far) difference compared to the article "getdata.Rmd" is that time 
here is measured in *days* instead of in a fraction of a year. *However*, "at the end", 
time is transformed back to units in years. The main reason is that the function `make.communal`
...
infdat <- infdat[order(infdat$id, infdat$enter), c("region", "id", "sex", "enter", "exit", "event", 
                                                   ...
                                                   "parity", tempvars)]
saveRDS(infdat, file = "data/infdat2_year2.rds")
```
