...
head(tneo)
summary(tneo)
saveRDS(tneo, file = "data/tneo.rds")
```

Note how it works: For each combination of *socStatus*, *illeg*, *parity* *cold*
(*cold* is temperature above minimum average), and *age*, the number of deaths (*event*)
and the *exposure* time (in days) are calculated. Note that the tabulated data frame is 
a tenth in size compared to the original file, and it is also *anonymised*.
