...
The way to disentangle the endogenous and exogenous componets is to start with a plot 
of the cumulative hazard function (Figure \@ref(fig:separ)).
...
axis(2, las = 1, pos = 0, at = c(0, 0.02, 0.04, 0.06), labels = c(0, 2, 4, 6))
#box()
abline(v = 0)
...
So we can see from Figure \@ref(fig:separ) that of the total mortality, 
`r round(5.27 / 7.63 * 100, 1)` per cent is exogenous and the rest is endogenous.
