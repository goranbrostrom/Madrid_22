---
title: "Infant mortality and temperature, northern Sweden 1895--1950"
...
tbl(head(infdat[, c("enter", "exit", "event", "margTemp.1", "expTemp.1", "week", "year")]),
...
fit.neow <- coxreg(Surv(enter, exit, event) ~ expTemp + expTemp.1 + expTemp.2 + expTemp.3 + expTemp.4 + 
                       margTemp + margTemp.1 + margTemp.2 + margTemp.3 + margTemp.4 + socst + 
...
##neo$margTemp <- neo$extemp.1
##neo$expTemp <- neo$emeantemp.1
fit.neos <- coxreg(Surv(enter, exit, event) ~ expTemp + margTemp + socst + 
                      sex + urban + parity + I(year - 1920),
                  data = neo[neo$summer, ])
...
During summer, expTemp and margTemp effects are clearly smaller, and regarding margTemp, the opposite 
...
The interaction between social status and *margTemp* is shown in Figure \@ref(fig:plotswsn). The same conclusion as 
for *expTemp*, nothing of interest shows up.

```{r plotswsn, fig.cap = "Socioeconomic status and margTemp, summer. Neonatal mortality.", fig.height = 3}
source("R/socfig.R")
...
##pneo$margTemp <- pneo$extemp.1
winter <- pneo[!pneo$summer, ]
fit.pneo <- coxreg(Surv(enter, exit, event) ~ expTemp + margTemp + sex + socst +
...
fit.pneos <- coxreg(Surv(enter, exit, event) ~ expTemp + margTemp + sex + socst + urban +
