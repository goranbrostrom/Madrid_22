...
knitr::opts_chunk$set(echo = FALSE, fig.path='figs/',fig.height=5,comment=NA)
library(knitr)
library(eha)
...
reason, whichever comes first. The first few lines (infants) are
...
We plot without the first step in the left hand panel and with it in the
right hand panel, see Figure \@ref(fig:ggplot2).

```{r ggplot2, fig.cap = "BB plots with and without time transform.", fig.height = 7}
nbths <- NROW(infdat)
...
plot(xt, ss / nbths, ylim = c(0, max(ss / nbths)), type = "l",
     ...
     main = "Untransformed", ylab = "Cum. deaths")
abline(h = 0)
axis(2, las = 1)
axis(1, at = att, labels = att)
box()
## g-transformed:
plot(g(xt), ss / nbths, ylim = c(0, max(ss / nbths)), type = "l",
     axes = FALSE, col = "blue", lwd = 2, xlab = "Age (days)",
