...
knitr::opts_chunk$set(echo = FALSE, fig.path='figs/',fig.height=3, comment=NA, global.par=TRUE)
library(knitr)
library(eha)
...
```{r monavgtemp, fig.cap="Weekly max and min temperature averages, 1895--1950.", fig.height = 3}
...
infdat$excessTemp <- infdat$cold # CHECK THIS!!
tbl(head(infdat))
```
...
We start by performing separate analyses for neonatal and postneonatal mortality. 
...
fit.tpch <- tpchreg(oe(event, exposure) ~ excessTemp + emintemp +  period + sex + socBranch + socStatus  + 
                        illeg + parity, 
                    data = tpneo, time = age)
dr <- drop1(fit.tpch, test = "Chisq")
ltx2(fit.tpch, dr = dr, caption = "Postneonatal mortality. Adjusted for social status and branch, illegitimacy, parity, time period, season, and subregion.", label = "tab:postanalske", keep = 2)
```
...
fit.ph <- coxreg(Surv(enter, exit, event) ~  (excessTemp + emintemp) * sex + socStatus + illeg + parity + period + socBranch + subreg,
                 data = neo)
dr <- drop1(fit.ph, test = "Chisq")
ltx2(fit.ph, dr = dr, caption = "Neonatal mortality. Adjusted for social status and branch, illegitimacy, parity, time period, and sub region.", label = "tab:neonatal", keep = 2)
```
...
fit.ph <- coxreg(Surv(enter, exit, event) ~  excessTemp + emintemp + socStatus + illeg + parity + period + socBranch + subreg,
                 data = neo[neo$sex == "boy" & neo$season %in% c("spring", "summer"), ])
dr <- drop1(fit.ph, test = "Chisq")
...
pneo$emintemp <- round(pneo$emintemp)
##
tpneo <- toTpch(Surv(enter, exit, event) ~ excessTemp + emintemp + sex + socBranch + socStatus + illeg + parity + 
                    period + subreg, 
                data = pneo, cuts = g(c(28, 365)))
##fit2 <- phreg(Surv(enter - g(28), exit - g(28), event) ~ sex + socBranch + socStatus + cold + emintemp, 
  ##           data = pneo, shape = 1)
##fit.pch <- pchreg(Surv(enter, exit, event) ~ sex + socBranch + socStatus + cold + emintemp, 
  ##           data = pneo, cuts = g(c(28, 365)))
fit.tpch <- tpchreg(oe(event, exposure) ~ excessTemp + emintemp + sex + socBranch + socStatus + illeg + parity + period + 
                        subreg, 
                    data = tpneo, time = age)
dr <- drop1(fit.tpch, test = "Chisq")
...
neo$enter <- g(neo$enter * 365)
fit.ph <- coxreg(Surv(enter, exit, event) ~  excessTemp + emintemp + socStatus + illeg + parity + period + socBranch + subreg,
                 data = neo[neo$sex == "girl" & neo$emeantemp < -0, ])
dr <- drop1(fit.ph, test = "Chisq")
...
fit.ph <- coxreg(Surv(enter, exit, event) ~  excessTemp + emintemp + socStatus + illeg + parity + period + socBranch + subreg,
                 data = neo[neo$sex == "boy" & neo$season %in% c("winter", "spring"), ])
dr <- drop1(fit.ph, test = "Chisq")
