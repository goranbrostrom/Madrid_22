...
through the statistical *sufficency principle*. 
...
If `event == FALSE`, then the infant died at age (in days) given by `exit`, otherwise
the infant survived (at least) 365 days and follow-up is lost at `exit == 365`. For the
time being, no covariates are involved. So, in Table \@ref(tab:infdat3), infants No. 1--4 survived, 
while infant No. 5 died at age 162 days.
...
divisions by *risk set size* instead of sample size should result in an approximately linear graph if the 
...
and then fit a piecewise constant hazards model with cutpoints at $g(0, 1, 2, 4, 7, 11, 16, 22, 28)$, see 
...
y <- hazards(fitall, cum = FALSE)$x
hy <- hazards(fitall, cum = FALSE)$y
te <- ginv(y)
hte <- dg(te) * hy
...
In accordance with the ideas of Bourgeouis-Pichat, we group the analyses into two parts, 
*neonatal* (first 28 days) och *postneonatal* mortality (days 29--365).

## Infant data

In the previous chapter we introduced data on infant mortality from the Umeå-Skellefteå
region 1895--1950, and here we introduce covariates of interest in the forthcoming  analyses, 
see Table 
