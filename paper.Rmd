...
empirical findings by Bourgeois-Pichat [@bp51a;  @bp51b] about endogenous and exogenous mortality and the log-cube transform,
so we start by recollecting his results and formulate them in a modern statistical setting.

# The Bourgeois-Pichat model

Bourgeois-Pichat (*B-P*) suggests that mortality
during the last eleven months of infancy follows a *uniform* distribution
given a specific transformation of time (age), the 
*log-cube transformation* $g$, see equation \@ref(eq:g), 
...
The B-P procedure consists of three steps:
...
data.  The transform data is stretched  stretched to 365 for easier comparison.

```{r ggplot2, fig.cap = "B-B plots with and without time transform, Umeå-Skellefteå 1895-1950.", fig.height = 4}
nbths <- NROW(infdat)
...
legend("topleft", legend = c("Cumulative hazards", "CDF (Bourgeois-Pichat)"), col = c("blue", "red"),
       lty = c(1, 2), cex = 0.7)
```
...
```{r excum, echo = FALSE}
fitex <- pchreg(Surv(enter, exit, event) ~ 1, cuts = g(c(28, 365)), data = ginfdat)
```

### Neonatal mortality

Neonatal mortality is, according to Bourgeois-Pichat, the sum of two components, *external* and *internal* causes. According to the *B-P*
theory, the external component is equal to the postneonatal mortality, which contains 
only an external component. 

We create the neonatal data set by censor all lives at the age 28 days:
...
```{r defcuts}
neocuts <- c(0, 2, 5, 8, 12, 17, 22, 25, 28)
pcuts <- c(neocuts, 365)
gneocuts <- g(neocuts)
gpcuts <- g(pcuts)
```
...
and then fit a piecewise constant hazards model with cutpoints at g(`r neocuts`), see 
Figure \@ref(fig:pchneo).

```{r pchneo, fig.cap = "Neonatal hazard function with external component (dashed).", fig.height = 3}
fitneo <- pchreg(Surv(enter, exit, event) ~ 1, data = neo, cuts = gneocuts)
plot(fitneo, fn = "haz", col = "blue", main = "", xlab = "Age (days)", axes = FALSE, lwd = 1.5)
axis(1, at = gneocuts, labels = neocuts)
axis(2, las = 1)
box()
abline(h = 0, v = 0)
...
par(las = 1)
plot(fitneo, fn = "haz", col = "blue", main = "", xlab = "Age (days)", lwd = 1.5,
     xlim = g(c(5, 28)), axes = FALSE)
axis(1, at = gneocuts, labels = neocuts)
axis(2)
box()
abline(h = 0, v = 0)
abline(h = posthaz, col = "red", lty = 2, lwd = 1.5)
```
...
constant hazards model with cutpoints at $g(0, 2, 5, 8, 12, 17, 22, 25, 28, 365)$, see 
Figure \@ref(fig:together).

```{r together, fig.cap = "Hazard function, infant mortality, transformed time.", fig.height=6}
ginfdat <- infdat
ginfdat$exit <- g(ginfdat$exit)
fitall <- pchreg(Surv(enter, exit, event) ~ 1, data = ginfdat, cuts = gpcuts)
plot(fitall, fn = "haz", col = "blue", main = "", xlab = "Age (days)", axes = FALSE,
     lwd = 1.5)

axis(1, at = gpcuts, labels = pcuts)
axis(2, las = 1)
box()
abline(h = 0, v = 0)
```
...
```{r allnorm, fig.cap = "Hazard function, infant mortality, natural time scale.", eval = FALSE}
...
The way to disentangle the endogenous and exogenous components is to start with a plot 
of the *cumulative* hazard function and add the exogenous 
part from the postneonatal estimate, see Figure \@ref(fig:separ).

```{r separ, fig.cap = "Separation of exogenous and endogenous infant mortality (cumulative hazards, per cent).", fig.width = 5, fig.height = 4}
plot(fitall, fn = "cum", col = "blue", main = "", xlab = "Age (days)", axes = FALSE,
     lwd = 1.5, ylab = "Cumulative hazards")

axis(1, at = c(0, g(28), g(365)), labels = c(0, 28, 365), pos = 0)
axis(2, las = 1, pos = 0, at = c(0, 0.0096, 0.0338), labels = c(0, 0.96, 3.38))
#box()
abline(v = 0)
texhaz <- g(365) * fitex$hazards[1, 1]
lines(c(0, g(365)), c(0, texhaz), col = "red", lwd = 1.5)
atte <- c(0.0000, 0.0763, texhaz)
axis(4, at = atte, labels = 100 * round(atte, 4), las = 1, cex = 0.5,
     pos = g(365))
abline(v = g(365))
text(100, 0.03, "Exogenous", col = "red", srt = 22, cex = 0.8)
text(100, 0.053, "Endogenous plus exogenous", col = "blue", srt = 22, cex = 0.8)
axis(3, at =  c(0, g(28), g(365)), labels = c(0, 28, 365))
lines(c(0, g(28)), c(0.0096, 0.0096), lty = 3)
lines(c(0, g(28)), c(0.0338, 0.0338), lty = 3)
abline(v = g(28))
```

So we can see from Figure \@ref(fig:separ) (vertical axis on the right hand side) that of the total mortality, 
`r round(5.27 / 7.63 * 100)` per cent is exogenous and the rest is endogenous.
Similarly, of the neonatal mortality, `r round(0.96 / 3.38 * 100)` per cent is
exogenous and the rest is endogenous (vertical axis on the left hand side).
As always, this requires that we trust the Bourgeois-Pichat model.
...
##fit.pch <- pchreg(Surv(enter, exit, event) ~ sex + socBranch + socStatus + cold + emintemp, 
  ##           data = pneo, cuts = g(c(28, 365)))
tpneo$icold <- with(tpneo, cold * (cold < 0))
fit.tpch <- tpchreg(oe(event, exposure) ~ icold + emintemp + season +  period + sex + socBranch + socStatus  + parity, 
                    data = tpneo[tpneo$subreg == "ske", ], time = age)
dr <- drop1(fit.tpch, test = "Chisq")
ltx(fit.tpch, dr = dr, caption = "Postneonatal mortality, Skellefteå.", label = "tab:postanalske")
```
...
fit.tpch <- tpchreg(oe(event, exposure) ~ icold + period + sex + socBranch + socStatus + period  + parity +
                        season, 
                    data = tpneo[tpneo$subreg == "ume", ], time = age)
dr <- drop1(fit.tpch, test = "Chisq")
ltx(fit.tpch, dr = dr, caption = "Postneonatal mortality, Umeå.", label = "tab:postanalume")
```
