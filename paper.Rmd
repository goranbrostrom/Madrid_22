...
formem <- as.formula("oe(event, exposure) ~ socst + socst:emeantemp + extemp + extemp.1 + urban + subreg + sex + parity")
...
fit.postw1 <- tpchreg(form, data = tabpost[(!tabpost$summer), ], time = age)
...
fit.postw1sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(!tabpost$summer) & tabpost$subreg == "ume", ], time = age)
dr.postw1sru <- drop1(fit.postw1sru, test = "Chisq")
```

```{r postw1sru, results = "asis"}
ltx2(fit.postw1sru, dr = dr.postw1sru, caption = "Postneonatal mortality, Umeå.",
...
ltx2(fit.postw1srs, dr = dr.postw1srs, caption = "Postneonatal mortality, Skellefteå with inland.", 
...
Figure \@ref(fig:socw1).

```{r socw1, fig.cap = "Interactions between social status and temperature, winter."}
source("R/socfig.R")
oldpar <- par(mfrow = c(1,3))
tempdat <- tabpost[(!tabpost$summer), ]
...
```{r posts1p, results = "asis"}
fit.posts1 <- tpchreg(form, data = tabpost[(tabpost$summer), ], time = age)
dr.posts1 <- drop1(fit.posts1, test = "Chisq")
```

```{r posts1, results = "asis", cache = FALSE}
ltx2(fit.posts1, dr = dr.posts1, caption = "Postneonatal mortality, October to March.", 
     label = "tab:posts1", keep = NULL)
```

Table \@ref(tab:posts1) shows that the effects of  extreme temperatures have the expected directions, extra high temperatures is *positive* 
(remember that this concerns summer conditions), but not statistically significant. *Climate* (emeantemp), on the other hand shows to be very
important, where increasing temperature by one degree lower mortality by 0.7 per cent. The normal short term fluctuations (extemp) are not so important, 
but in the expected direction. 
...
```{r posts1srpu, cache = TRUE}
fit.posts1sru <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(tabpost$summer) & tabpost$subreg == "ume", ], time = age)
dr.posts1sru <- drop1(fit.posts1sru, test = "Chisq")
```

```{r posts1sru, results = "asis"}
ltx2(fit.posts1sru, dr = dr.posts1sru, caption = "Postneonatal mortality, Umeå, October to March.",
     label = "tab:posts1sru", keep = NULL)
```

```{r posts1srps, cache = TRUE}
fit.posts1srs <- tpchreg(oe(event, exposure) ~ extemp + extemp.1 + emeantemp + socst + urban + sex + parity, 
                        data = tabpost[(tabpost$summer) & tabpost$subreg != "ume", ], time = age)
dr.posts1srs <- drop1(fit.posts1srs, test = "Chisq")
```

```{r posts1srs, results = "asis"}
ltx2(fit.posts1srs, dr = dr.posts1srs, caption = "Postneonatal mortality, Skellefteå with inland, October to March.", 
     label = "tab:posts1srs", keep = NULL)
```

We can conclude from Tables \@ref(tab:posts1sru) and \@ref(tab:posts1srs) that the main difference between Umeå and Skellefteå is that
*climate* has more severe effect in Skellefteå (with inland), while *weather* is less important.

The effects of temperature in different social classes are best shown graphically, see 
Figure \@ref(fig:soc2).

```{r soc1, fig.cap = "Interactions between social status and temperature, summer."}
source("R/socfig.R")
oldpar <- par(mfrow = c(1,3))
tempdat <- tabpost[(tabpost$summer), ]
fit <- tpchreg(formem, data = tempdat, time = age)
