ltx2 <- function(x, caption = NULL, label = NULL, dr = NULL,
                 digits = max(options()$digits -4, 3),
                 keep = NULL, ...){
    
    ## A variation of ltx.tpchreg (for the moment)
    ## where some parameter estimates (corresponding to
    ## variables, not levels) can be supressed.
    ##
    ## If keep == "all", look at exclude.
    ## If keep != "all", don't look at exclude.

    ## Use keep for the moment

    ##if (!inherits(x, "coxreg")){
      ##  stop("only for coxreg objects")
    ##}

    if (is.null(keep)) keep <- 1000 # Something BIG!
    ...
    ltxCoef2(x, digits, dr, caption, keep) # Print regression coefficients
### New 2020-11-26:    
...
ltxCoef2 <- function(x, digits, dr, caption, keep){
    coef <- x$coef
...
#####################################
    ...
    nterms <- min(keep, length(term.names))
    ##for (term.no in 1:length(term.names)){
    for (term.no in 1:nterms){
